function ChatHeader({ role, toggleMobileMenu, handleLogout, isProgrammer, clearChat }) {
  return (
    <div className="bg-slate-800 p-3 sm:p-4 border-b border-slate-700 flex justify-between items-center">
      ...
      <div className="flex items-center gap-2">
        {/* Кнопка очистки чата — видна только программисту */}
        {isProgrammer && (
          <button
            onClick={clearChat}
            title="Очистить чат"
          ...
          >
            🗑️ <span className="hidden sm:inline">Очистить</span>
          </button>
        )}
